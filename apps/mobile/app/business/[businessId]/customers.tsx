...
function message(error: unknown): string {
  return error instanceof Error && error.message ? error.message : 'تعذر تحميل العملاء.';
}
...
export default function CustomersScreen() {
  ...
  const load = useCallback(() => {
    ...
    setError(null);
    void ibex
      .listBusinessCustomers({ businessId, search })
      .then((rows) => {
      ...
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        ...
        {!loading && customers.length === 0 ? (
          <View style={styles.emptyCard}>
            ...
            <Text style={styles.emptyBody}>أضف أول عميل أو غيّر عبارة البحث.</Text>
          </View>
        ) : null}
        ...
        <View style={styles.list}>
          {customers.map((customer) => (
            <Pressable
              ...
              style={({ pressed }) => [styles.customerCard, pressed ? styles.cardPressed : null]}
            >
              <View style={styles.customerMeta}>
                <Text style={styles.customerName}>{customer.displayName}</Text>
                <Text style={styles.customerCaption}>
                  {customer.phoneE164 ?? 'بدون رقم جوال'} · {customer.accountCount} حساب
                </Text>
              </View>
