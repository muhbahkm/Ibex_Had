...
function priorityLabel(row: CollectionTodayPlanRecord): string {
  if (row.priorityBucket === 'urgent') return 'عاجل';
  if (row.priorityBucket === 'high') return 'أولوية عالية';
  if (row.priorityBucket === 'medium') return 'متابعة اليوم';
  return 'استعداد';
}
  ...
  return (
    <main className="shell">
      ...
      <section className="hero compact-hero">
        ...
        <p>تابع الذمم والاستحقاقات وسجل التواصل من Read Models تشغيلية قابلة للتفسير وتحافظ على فصل العملات.</p>
      </section>
      ...
      <div className="grid">
        ...
        <section className="card span-12">
          <div className="section-head"><div><h2 className="section-title">خطة اليوم</h2><p className="section-subtitle">ترتيب حتمي حسب الاستحقاق وتاريخ التواصل، وليس حسب مقارنة مبالغ العملات.</p></div></div>
          <div className="list">
            {todayPlan.slice(0, 8).map((row) => (
              <a className="row-card interactive-row" key={row.accountId} href={`/dashboard/business/${businessId}/customer/${row.businessCustomerId}/${row.customerIdentityId}`}>
                <div className="row-main">
                  ...
                  <p className="row-meta">{actionLabel(row.recommendedAction)} · استحقاق {new Date(row.oldestDueAt).toLocaleDateString('en-GB')} · درجة {String(row.priorityScore)}</p>
                  <p className="row-meta">{row.lastFollowUpAt ? `آخر متابعة ${new Date(row.lastFollowUpAt).toLocaleDateString('en-GB')}` : 'لا توجد متابعة سابقة'}{row.promisedFor ? ` · وعد ${new Date(`${row.promisedFor}T00:00:00`).toLocaleDateString('en-GB')}` : ''}</p>
                </div>
      ...
      <div className="grid">
        <section className="card span-8">
          <div className="section-head"><div><h2 className="section-title">العملاء</h2><p className="section-subtitle">{customers.length} نتيجة</p></div><form className="inline-form" onSubmit={(event) => void submitSearch(event)}><input aria-label="بحث العملاء" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="الاسم أو رقم الجوال" /><button className="button secondary compact-button" disabled={loading}>بحث</button></form></div>
          <div className="list">{loading ? <div className="row-card muted">جارٍ التحميل…</div> : null}{!loading && customers.map((customer) => <a className="row-card interactive-row" key={customer.businessCustomerId} href={`/dashboard/business/${businessId}/customer/${customer.businessCustomerId}/${customer.customerIdentityId}`}><div className="row-main"><p className="row-title">{customer.displayName}</p><p className="row-meta">{customer.phoneE164 ?? 'بدون رقم جوال'} · {customer.accountCount} حساب</p></div><span className="badge">فتح الحساب</span></a>)}{!loading && customers.length === 0 ? <div className="row-card muted">لا توجد نتائج مطابقة.</div> : null}</div>
        </section>
