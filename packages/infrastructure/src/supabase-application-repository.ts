...
export class SupabaseApplicationRepository implements ApplicationRepository {
  ...
  async createCustomerInvite(input: CreateCustomerInvitePortInput): Promise<CustomerInviteRecord> {
    ...
    return {
      inviteId: expectString(row.inviteId, 'inviteId', operation),
      token: expectString(row.token, 'token', operation),
      expiresAt: expectString(row.expiresAt, 'expiresAt', operation),
  ...
  async claimCustomerInvite(input: ClaimCustomerInvitePortInput): Promise<ClaimedCustomerInviteRecord> {
    ...
    return {
      businessId: expectString(row.businessId, 'businessId', operation),
      businessName: expectString(row.businessName, 'businessName', operation),
      businessCustomerId: expectString(row.businessCustomerId, 'businessCustomerId', operation),
      customerIdentityId: expectString(row.customerIdentityId, 'customerIdentityId', operation),
    };
  }
